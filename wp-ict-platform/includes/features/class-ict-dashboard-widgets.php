...
class ICT_Dashboard_Widgets {
    ...
    public function register_routes() {
        register_rest_route( 'ict/v1', '/dashboard/widgets', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'get_available_widgets' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/dashboard/layout', array(
            array(
                'methods'             => WP_REST_Server::READABLE,
        ...
        register_rest_route( 'ict/v1', '/dashboard/layout/reset', array(
            'methods'             => WP_REST_Server::CREATABLE,
            'callback'            => array( $this, 'reset_layout' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/dashboard/widget/(?P<widget_id>[a-z_]+)/data', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'get_widget_data' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/dashboard/widget/(?P<widget_id>[a-z_]+)/settings', array(
            array(
                'methods'             => WP_REST_Server::READABLE,
        ...
        register_rest_route( 'ict/v1', '/dashboard/summary', array(
            'methods'             => WP_REST_Server::READABLE,
    ...
    public function get_available_widgets( $request ) {
        ...
        $user_widgets = array_filter( $widgets, function( $widget ) {
    ...
    public function get_layout( $request ) {
        ...
        $layout = $wpdb->get_row( $wpdb->prepare(
            "SELECT * FROM {$this->layouts_table} WHERE user_id = %d", $user_id
        ) );
    ...
    public function save_layout( $request ) {
        ...
        $existing = $wpdb->get_var( $wpdb->prepare(
            "SELECT id FROM {$this->layouts_table} WHERE user_id = %d", $user_id
        ) );
    ...
    public function get_dashboard_summary( $request ) {
        ...
        // Today's hours
        $summary['today_hours'] = $wpdb->get_var( $wpdb->prepare(
        ...
        // Pending tasks
        $summary['pending_tasks'] = $wpdb->get_var( $wpdb->prepare(
        ...
        // Unread messages
        $summary['unread_messages'] = $wpdb->get_var( $wpdb->prepare(
        ...
        // Pending approvals
        $summary['pending_approvals'] = $wpdb->get_var( $wpdb->prepare(
    ...
    public function render_wp_summary_widget() {
        ...
        $today_hours = $wpdb->get_var( $wpdb->prepare(
            "SELECT COALESCE(SUM(TIMESTAMPDIFF(MINUTE, start_time, COALESCE(end_time, NOW()))), 0) / 60
             FROM {$wpdb->prefix}ict_time_entries
             WHERE user_id = %d AND DATE(start_time) = CURDATE()",
            $user_id
    ...
    private function get_pending_approvals_data( $params ) {
        ...
        $time_approvals = $wpdb->get_var( $wpdb->prepare(
            "SELECT COUNT(*) FROM {$wpdb->prefix}ict_time_approvals
             WHERE approver_id = %d AND status = 'pending'",
            $user_id
        ) );

        $po_approvals = $wpdb->get_var( $wpdb->prepare(
    ...
    private function get_quick_actions_data( $params ) {
        return array(
            'actions' => array(
                array( 'id' => 'clock_in', 'label' => 'Clock In', 'icon' => 'play' ),
                array( 'id' => 'new_task', 'label' => 'New Task', 'icon' => 'plus' ),
                array( 'id' => 'log_time', 'label' => 'Log Time', 'icon' => 'clock' ),
                array( 'id' => 'scan_item', 'label' => 'Scan Item', 'icon' => 'barcode' ),
            ),
        );
    }

...
class ICT_Import_Wizard {
    ...
    public function register_routes() {
        register_rest_route( 'ict/v1', '/import/upload', array(
            'methods'             => WP_REST_Server::CREATABLE,
            'callback'            => array( $this, 'upload_file' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/import/(?P<id>\d+)/preview', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'preview_import' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/import/(?P<id>\d+)/mapping', array(
            array(
                'methods'             => WP_REST_Server::READABLE,
        ...
        register_rest_route( 'ict/v1', '/import/(?P<id>\d+)/validate', array(
            'methods'             => WP_REST_Server::CREATABLE,
            'callback'            => array( $this, 'validate_import' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/import/(?P<id>\d+)/run', array(
            'methods'             => WP_REST_Server::CREATABLE,
            'callback'            => array( $this, 'run_import' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/import/(?P<id>\d+)/status', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'get_status' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/import/history', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'get_history' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/import/templates', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'get_templates' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/import/templates/(?P<type>[a-z_]+)', array(
            'methods'             => WP_REST_Server::READABLE,
    ...
    public function upload_file( $request ) {
        ...
        $wpdb->insert( $this->imports_table, array(
    ...
    public function preview_import( $request ) {
        ...
        $import = $wpdb->get_row( $wpdb->prepare(
            "SELECT * FROM {$this->imports_table} WHERE id = %d", $id
        ) );
    ...
    public function get_mapping( $request ) {
        ...
        $import = $wpdb->get_row( $wpdb->prepare(
            "SELECT import_type, column_mapping FROM {$this->imports_table} WHERE id = %d", $id
        ) );
    ...
    public function save_mapping( $request ) {
        ...
        $wpdb->update( $this->imports_table, array(
    ...
    public function validate_import( $request ) {
        ...
        $wpdb->update( $this->imports_table, array(
            'status'            => $status,
            'validation_errors' => wp_json_encode( array(
                'errors'   => $errors,
                'warnings' => $warnings,
            ) ),
        ), array( 'id' => $id ) );
    ...
    private function get_target_fields( $import_type ) {
        $fields = array(
            'projects' => array(
                array( 'key' => 'project_number', 'label' => 'Project Number', 'required' => false ),
                array( 'key' => 'name', 'label' => 'Project Name', 'required' => true ),
                array( 'key' => 'description', 'label' => 'Description', 'required' => false ),
                array( 'key' => 'client_id', 'label' => 'Client ID', 'required' => false ),
                array( 'key' => 'start_date', 'label' => 'Start Date', 'required' => false ),
                array( 'key' => 'end_date', 'label' => 'End Date', 'required' => false ),
                array( 'key' => 'budget', 'label' => 'Budget', 'required' => false ),
                array( 'key' => 'status', 'label' => 'Status', 'required' => false ),
            ),
            'inventory' => array(
                array( 'key' => 'sku', 'label' => 'SKU', 'required' => true ),
                array( 'key' => 'name', 'label' => 'Item Name', 'required' => true ),
                array( 'key' => 'description', 'label' => 'Description', 'required' => false ),
                array( 'key' => 'category', 'label' => 'Category', 'required' => false ),
                array( 'key' => 'quantity', 'label' => 'Quantity', 'required' => true ),
                array( 'key' => 'unit_cost', 'label' => 'Unit Cost', 'required' => false ),
                array( 'key' => 'reorder_point', 'label' => 'Reorder Point', 'required' => false ),
            ),
            'suppliers' => array(
                array( 'key' => 'name', 'label' => 'Supplier Name', 'required' => true ),
                array( 'key' => 'code', 'label' => 'Supplier Code', 'required' => false ),
                array( 'key' => 'type', 'label' => 'Type', 'required' => false ),
                array( 'key' => 'email', 'label' => 'Email', 'required' => false ),
                array( 'key' => 'phone', 'label' => 'Phone', 'required' => false ),
                array( 'key' => 'address_line1', 'label' => 'Address', 'required' => false ),
                array( 'key' => 'city', 'label' => 'City', 'required' => false ),
                array( 'key' => 'state', 'label' => 'State', 'required' => false ),
                array( 'key' => 'country', 'label' => 'Country', 'required' => false ),
            ),
        );
    ...
    private function validate_row( $row, $mapping, $import_type, $row_num ) {
        ...
        $required = array_filter( $fields, function( $f ) { return $f['required']; } );

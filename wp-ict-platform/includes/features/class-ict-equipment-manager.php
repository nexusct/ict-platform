...
class ICT_Equipment_Manager {
    ...
    public function register_rest_routes() {
        ...
        // Equipment endpoints.
        register_rest_route( $namespace, '/equipment', array(
            array(
        ...
        register_rest_route( $namespace, '/equipment/(?P<id>\d+)', array(
            array(
                'methods'             => WP_REST_Server::READABLE,
        ...
        register_rest_route( $namespace, '/equipment/(?P<id>\d+)/assign', array(
            'methods'             => WP_REST_Server::CREATABLE,
            'callback'            => array( $this, 'rest_assign_equipment' ),
            'permission_callback' => array( $this, 'check_manage_permission' ),
        ) );

        register_rest_route( $namespace, '/equipment/(?P<id>\d+)/return', array(
            'methods'             => WP_REST_Server::CREATABLE,
        ...
        // Checkout endpoints.
        register_rest_route( $namespace, '/equipment/(?P<id>\d+)/checkout', array(
            'methods'             => WP_REST_Server::CREATABLE,
            'callback'            => array( $this, 'rest_checkout_equipment' ),
            'permission_callback' => array( $this, 'check_read_permission' ),
        ) );

        register_rest_route( $namespace, '/equipment/(?P<id>\d+)/checkin', array(
            'methods'             => WP_REST_Server::CREATABLE,
        ...
        register_rest_route( $namespace, '/equipment/(?P<id>\d+)/maintenance', array(
            array(
                'methods'             => WP_REST_Server::READABLE,
        ...
        // Categories.
        register_rest_route( $namespace, '/equipment-categories', array(
            array(
                'methods'             => WP_REST_Server::READABLE,
        ...
        // Barcode/QR lookup.
        register_rest_route( $namespace, '/equipment/lookup', array(
            'methods'             => WP_REST_Server::READABLE,
        ...
        // Reports.
        register_rest_route( $namespace, '/equipment/reports/depreciation', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'rest_depreciation_report' ),
            'permission_callback' => array( $this, 'check_manage_permission' ),
        ) );

        register_rest_route( $namespace, '/equipment/reports/utilization', array(
            'methods'             => WP_REST_Server::READABLE,
    ...
    public function rest_get_equipment( $request ) {
        ...
        // Count total.
        $count_sql = str_replace( 'SELECT e.*, c.name as category_name,
                (SELECT assigned_to FROM ' . $this->tables['assignments'] . '
                 WHERE equipment_id = e.id AND returned_at IS NULL
                 ORDER BY assigned_at DESC LIMIT 1) as current_assignee', 'SELECT COUNT(*)', $sql );
    ...
    public function rest_get_equipment_item( $request ) {
        ...
        $equipment->current_assignment = $wpdb->get_row( $wpdb->prepare(
        ...
        $equipment->recent_maintenance = $wpdb->get_results( $wpdb->prepare(
        ...
        $equipment->checkout_history = $wpdb->get_results( $wpdb->prepare(
    ...
    public function rest_create_equipment( $request ) {
        ...
        $data['qr_code'] = wp_json_encode( array(
            'type' => 'ict_equipment',
            'id'   => 'PENDING',
        ...
        $wpdb->update(
            ...
            array( 'qr_code' => wp_json_encode( array(
                'type' => 'ict_equipment',
                'id'   => $equipment_id,
                'sn'   => $data['serial_number'],
            ) ) ),
    ...
    public function rest_update_equipment( $request ) {
        ...
        $fields = array(
            'name', 'description', 'category_id', 'serial_number', 'model_number',
            'manufacturer', 'barcode', 'purchase_date', 'purchase_price', 'current_value',
            'warranty_expiry', 'status', 'condition_rating', 'location', 'notes',
            'next_maintenance_date', 'next_calibration_date'
        );

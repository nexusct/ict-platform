...
class ICT_KPI_Tracker {
	...
	public function get_kpis( $request ) {
		...
		// Get latest values
		foreach ( $kpis as $kpi ) {
			$kpi->current_value = $wpdb->get_var(
				$wpdb->prepare(
			...
			$kpi->current_target = $wpdb->get_var(
				$wpdb->prepare(
	...
	public function get_kpi( $request ) {
		...
		$kpi = $wpdb->get_row(
			$wpdb->prepare(
				"SELECT * FROM {$this->kpis_table} WHERE id = %d",
				$id
			)
		);
	...
	public function create_kpi( $request ) {
		...
		$existing = $wpdb->get_var(
			$wpdb->prepare(
				"SELECT id FROM {$this->kpis_table} WHERE slug = %s",
				$slug
			)
		);
	...
	public function get_dashboard( $request ) {
		...
		foreach ( $kpis as $kpi ) {
			...
			if ( $kpi->critical_threshold && $current ) {
				...
				} elseif ( $current->value >= $kpi->critical_threshold ) {
						$status = 'critical';
				} elseif ( $kpi->warning_threshold && $current->value >= $kpi->warning_threshold ) {
					$status = 'warning';
				}

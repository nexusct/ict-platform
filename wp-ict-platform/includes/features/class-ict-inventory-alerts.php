...
class ICT_Inventory_Alerts {
    ...
    public function register_routes() {
        register_rest_route( 'ict/v1', '/inventory/alerts', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'get_alerts' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/inventory/alerts/(?P<id>\d+)/acknowledge', array(
            'methods'             => WP_REST_Server::CREATABLE,
            'callback'            => array( $this, 'acknowledge_alert' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/inventory/low-stock', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'get_low_stock' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/inventory/scan', array(
            'methods'             => WP_REST_Server::CREATABLE,
            'callback'            => array( $this, 'scan_barcode' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/inventory/items/(?P<item_id>\d+)/barcode', array(
            array(
                'methods'             => WP_REST_Server::READABLE,
        ...
        register_rest_route( 'ict/v1', '/inventory/items/(?P<item_id>\d+)/adjust', array(
            'methods'             => WP_REST_Server::CREATABLE,
            'callback'            => array( $this, 'adjust_stock' ),
            'permission_callback' => array( $this, 'check_edit_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/inventory/items/(?P<item_id>\d+)/history', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'get_history' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/inventory/reorder-suggestions', array(
            'methods'             => WP_REST_Server::READABLE,
    ...
    public function acknowledge_alert( $request ) {
        ...
        $wpdb->update( $this->alerts_table, array(
            'is_acknowledged' => 1,
    ...
    public function scan_barcode( $request ) {
        ...
        $record = $wpdb->get_row( $wpdb->prepare(
            "SELECT * FROM {$this->barcodes_table} WHERE barcode = %s", $barcode
        ) );

        if ( ! $record ) {
            $item = $wpdb->get_row( $wpdb->prepare(
                "SELECT * FROM {$wpdb->prefix}ict_inventory_items WHERE sku = %s", $barcode
            ) );
            if ( ! $item ) {
                return rest_ensure_response( array( 'found' => false, 'barcode' => $barcode ) );
            }
        } else {
            $item = $wpdb->get_row( $wpdb->prepare(
                "SELECT * FROM {$wpdb->prefix}ict_inventory_items WHERE id = %d", $record->item_id
            ) );
        }

        $result = array( 'found' => true, 'item' => $item );
    ...
    public function get_barcode( $request ) {
        ...
        $barcodes = $wpdb->get_results( $wpdb->prepare(
            "SELECT * FROM {$this->barcodes_table} WHERE item_id = %d", $item_id
        ) );
    ...
    public function set_barcode( $request ) {
        ...
        $wpdb->insert( $this->barcodes_table, array(
            'item_id'      => $item_id,
            'barcode'      => $barcode,
            'barcode_type' => $type,
        ) );
    ...
    public function adjust_stock( $request ) {
        ...
        $item = $wpdb->get_row( $wpdb->prepare(
            "SELECT * FROM {$wpdb->prefix}ict_inventory_items WHERE id = %d", $item_id
        ) );
    ...
    public function get_reorder_suggestions( $request ) {
        ...
        return rest_ensure_response( array( 'items' => $items, 'total_estimated_cost' => $total ) );
    }
    ...
    public function check_stock_levels( $item_id, $data = null ) {
        ...
        $wpdb->delete( $this->alerts_table, array( 'item_id' => $item_id, 'is_acknowledged' => 0 ) );

        if ( $item->quantity <= 0 ) {
            $this->create_alert( $item_id, 'out_of_stock', 'critical',
                "{$item->name} is out of stock", $item->quantity, 0 );
            ...
            $this->create_alert( $item_id, 'low_stock', $severity,
                "{$item->name} is low ({$item->quantity} remaining)", $item->quantity, $item->reorder_point );
        }
    }

    private function create_alert( $item_id, $type, $severity, $message, $current, $threshold ) {
        global $wpdb;
        $wpdb->insert( $this->alerts_table, array(
            'item_id'       => $item_id,
            'alert_type'    => $type,
            'severity'      => $severity,
            'message'       => $message,
    ...
    private function record_movement( $item_id, $type, $qty, $reason ) {
        global $wpdb;
        $item = $wpdb->get_row( $wpdb->prepare(
            "SELECT quantity FROM {$wpdb->prefix}ict_inventory_items WHERE id = %d", $item_id
        ) );

        $wpdb->insert( $this->movements_table, array(
            'item_id'         => $item_id,
            'movement_type'   => $type,
            'quantity'        => $qty,

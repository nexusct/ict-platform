...
class ICT_SLA_Tracking {
    ...
    public function register_routes() {
        register_rest_route( 'ict/v1', '/slas', array(
            array(
        ...
        register_rest_route( 'ict/v1', '/slas/(?P<id>\d+)', array(
            array(
                'methods'             => WP_REST_Server::READABLE,
        ...
        register_rest_route( 'ict/v1', '/slas/(?P<id>\d+)/breaches', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'get_breaches' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/slas/(?P<id>\d+)/metrics', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'get_metrics' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/slas/dashboard', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'get_dashboard' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/slas/at-risk', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'get_at_risk' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/slas/client/(?P<client_id>\d+)', array(
            'methods'             => WP_REST_Server::READABLE,
    ...
    public function get_sla( $request ) {
        ...
        $sla = $wpdb->get_row( $wpdb->prepare(
            "SELECT * FROM {$this->slas_table} WHERE id = %d", $id
        ) );
        ...
        // Get recent metrics
        $sla->metrics = $wpdb->get_results( $wpdb->prepare(
        ...
        // Get active breaches
        $sla->active_breaches = $wpdb->get_results( $wpdb->prepare(
    ...
    public function update_sla( $request ) {
        ...
        $fields = array( 'name', 'description', 'applies_to', 'applies_to_value',
            'business_start', 'business_end', 'business_days' );
        ...
        foreach ( $response_slas as $sla ) {
            ...
            $tickets = $wpdb->get_results( $wpdb->prepare(
                "SELECT * FROM {$wpdb->prefix}ict_tickets
                 ...
                 AND TIMESTAMPDIFF(MINUTE, created_at, NOW()) < %d",
                $warning_minutes, $threshold_minutes
            ) );
    ...
    public function get_client_slas( $request ) {
        ...
        $slas = $wpdb->get_results( $wpdb->prepare(
            "SELECT s.*,
                    (SELECT compliance_rate FROM {$this->metrics_table}
                     WHERE sla_id = s.id ORDER BY period_end DESC LIMIT 1) as current_compliance
             FROM {$this->slas_table} s
    ...
    private function check_response_time_sla( $sla ) {
        ...
        foreach ( $breached as $ticket ) {
            ...
            if ( ! $existing ) {
                $wpdb->insert( $this->breaches_table, array(
                    'sla_id'       => $sla->id,
                    'entity_type'  => 'ticket',
                    'entity_id'    => $ticket->id,
                    'breach_type'  => 'breach',
    ...
    private function calculate_daily_metrics() {
        ...
        foreach ( $slas as $sla ) {
            ...
            $breached = $wpdb->get_var( $wpdb->prepare(
                "SELECT COUNT(*) FROM {$this->breaches_table}
            ...
            $wpdb->replace( $this->metrics_table, array(
                'sla_id'          => $sla->id,
                'period_start'    => $today,
                'period_end'      => $today,
                'total_items'     => $total,

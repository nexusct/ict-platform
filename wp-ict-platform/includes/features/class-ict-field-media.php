...
class ICT_Field_Media {
	...
	public function register_routes() {
		...
		register_rest_route(
			'ict/v1',
			'/media/voice-notes/(?P<id>\d+)/transcribe',
			array(
				'methods'             => WP_REST_Server::CREATABLE,
		...
		register_rest_route(
			'ict/v1',
			'/media/signatures/verify/(?P<token>[a-zA-Z0-9]+)',
			array(
				'methods'             => WP_REST_Server::READABLE,
		...
		register_rest_route(
			'ict/v1',
			'/media/(?P<entity_type>[a-z_]+)/(?P<entity_id>\d+)',
			array(
				'methods'             => WP_REST_Server::READABLE,
	...
	public function delete_voice_note( $request ) {
		...
		$note = $wpdb->get_row(
			$wpdb->prepare(
				"SELECT file_path FROM {$this->media_table} WHERE id = %d",
				$id
			)
		);

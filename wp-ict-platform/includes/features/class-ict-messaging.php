...
class ICT_Messaging {
    ...
    public function register_routes() {
        register_rest_route( 'ict/v1', '/conversations', array(
            array(
        ...
        register_rest_route( 'ict/v1', '/conversations/(?P<id>\d+)', array(
            array(
                'methods'             => WP_REST_Server::READABLE,
        ...
        register_rest_route( 'ict/v1', '/conversations/(?P<id>\d+)/messages', array(
            array(
                'methods'             => WP_REST_Server::READABLE,
        ...
        register_rest_route( 'ict/v1', '/conversations/(?P<id>\d+)/read', array(
            'methods'             => WP_REST_Server::CREATABLE,
            'callback'            => array( $this, 'mark_read' ),
            'permission_callback' => array( $this, 'check_conversation_access' ),
        ) );

        register_rest_route( 'ict/v1', '/conversations/(?P<id>\d+)/participants', array(
            array(
                'methods'             => WP_REST_Server::READABLE,
        ...
        register_rest_route( 'ict/v1', '/messages/unread-count', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'get_unread_count' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/messages/direct', array(
            'methods'             => WP_REST_Server::CREATABLE,
            'callback'            => array( $this, 'send_direct_message' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/messages/search', array(
            'methods'             => WP_REST_Server::READABLE,
    ...
    public function get_conversation( $request ) {
        ...
        $conv = $wpdb->get_row( $wpdb->prepare(
            "SELECT * FROM {$this->conversations_table} WHERE id = %d", $id
        ) );
    ...
    public function create_conversation( $request ) {
        ...
        if ( $type === 'direct' && count( $participants ) === 2 ) {
            ...
            if ( $existing ) {
                return rest_ensure_response( array( 'success' => true, 'id' => $existing, 'existing' => true ) );
            }
        }
        ...
        foreach ( $participants as $pid ) {
            $wpdb->insert( $this->participants_table, array(
                'conversation_id' => $conv_id,
                'user_id'         => (int) $pid,
            ) );
        }

        return rest_ensure_response( array( 'success' => true, 'id' => $conv_id ) );
    }
    ...
    public function send_message( $request ) {
        ...
        $wpdb->update( $this->conversations_table, array(
    ...
    public function send_direct_message( $request ) {
        ...
        if ( ! $conv_id ) {
            ...
            $wpdb->insert( $this->participants_table, array( 'conversation_id' => $conv_id, 'user_id' => $user_id ) );
            $wpdb->insert( $this->participants_table, array( 'conversation_id' => $conv_id, 'user_id' => $recipient_id ) );
        }

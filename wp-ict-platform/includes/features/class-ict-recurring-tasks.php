...
class ICT_Recurring_Tasks {
    ...
    public function register_routes() {
        register_rest_route( 'ict/v1', '/recurring-tasks', array(
            array(
        ...
        register_rest_route( 'ict/v1', '/recurring-tasks/(?P<id>\d+)', array(
            array(
                'methods'             => WP_REST_Server::READABLE,
        ...
        register_rest_route( 'ict/v1', '/recurring-tasks/(?P<id>\d+)/pause', array(
            'methods'             => WP_REST_Server::CREATABLE,
            'callback'            => array( $this, 'pause_template' ),
            'permission_callback' => array( $this, 'check_edit_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/recurring-tasks/(?P<id>\d+)/resume', array(
            'methods'             => WP_REST_Server::CREATABLE,
            'callback'            => array( $this, 'resume_template' ),
            'permission_callback' => array( $this, 'check_edit_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/recurring-tasks/(?P<id>\d+)/generate', array(
            'methods'             => WP_REST_Server::CREATABLE,
            'callback'            => array( $this, 'generate_instance' ),
            'permission_callback' => array( $this, 'check_edit_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/recurring-tasks/(?P<id>\d+)/instances', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'get_instances' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/recurring-tasks/(?P<id>\d+)/history', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'get_history' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/recurring-tasks/upcoming', array(
            'methods'             => WP_REST_Server::READABLE,
    ...
    public function get_template( $request ) {
        ...
        $template = $wpdb->get_row( $wpdb->prepare(
            "SELECT * FROM {$this->templates_table} WHERE id = %d", $id
        ) );
        ...
        $template->upcoming_instances = $wpdb->get_results( $wpdb->prepare(
    ...
    public function create_template( $request ) {
        ...
        return rest_ensure_response( array( 'success' => true, 'id' => $template_id ) );
    }
    ...
    public function update_template( $request ) {
        ...
        $fields = array( 'name', 'description', 'task_type', 'frequency_type', 'frequency_value',
            'days_of_week', 'day_of_month', 'month_of_year', 'cron_expression', 'start_date', 'end_date',
            'estimated_hours', 'priority', 'notify_before_hours', 'auto_assign', 'max_occurrences' );
    ...
    public function resume_template( $request ) {
        ...
        return rest_ensure_response( array( 'success' => true, 'next_occurrence' => $next ) );
    }
    ...
    public function generate_instance( $request ) {
        ...
        return rest_ensure_response( array( 'success' => true, 'instance_id' => $instance_id ) );
    }
    ...
    public function process_due_tasks() {
        ...
        foreach ( $templates as $template ) {
            ...
            if ( $template->max_occurrences && $template->occurrences_count >= $template->max_occurrences ) {
                $wpdb->update( $this->templates_table,
    ...
    private function create_instance( $template, $scheduled_date ) {
        ...
        $wpdb->insert( $this->instances_table, array(
        ...
        $this->log_history( $template->id, $instance_id, 'instance_created',

...
class ICT_Global_Search {
    ...
    public function register_routes() {
        register_rest_route( 'ict/v1', '/search', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'search' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/search/quick', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'quick_search' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/search/suggestions', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'get_suggestions' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/search/history', array(
            array(
                'methods'             => WP_REST_Server::READABLE,
        ...
        register_rest_route( 'ict/v1', '/search/reindex', array(
    ...
    public function reindex_all( $request ) {
        ...
        foreach ( $suppliers as $supplier ) {
            $this->add_to_index( 'supplier', $supplier->id, array(
                'title'    => $supplier->name,
    ...
    public function index_project( $project_id, $data ) {
        $this->add_to_index( 'project', $project_id, array(
            'title'    => $data['name'],
    ...
    public function index_inventory( $item_id, $data ) {
        $this->add_to_index( 'inventory', $item_id, array(
            'title'    => $data['name'],
            'content'  => $data['description'] ?? '',
    ...
    public function index_document( $doc_id, $data ) {
        $this->add_to_index( 'document', $doc_id, array(
            'title'    => $data['name'],
            'content'  => $data['description'] ?? '',
            'keywords' => $data['tags'] ?? '',
            'metadata' => wp_json_encode( array(
    ...
    private function add_to_index( $entity_type, $entity_id, $data ) {
        ...
        $wpdb->replace( $this->search_index_table, array(
        ...
        if ( ! $type_array || in_array( 'project', $type_array, true ) ) {
            ...
            foreach ( $projects as $p ) {
                $results[] = array(
                    ...
                    'metadata' => array( 'status' => $p->status, 'number' => $p->project_number ),
        ...
        if ( ! $type_array || in_array( 'inventory', $type_array, true ) ) {
            ...
            foreach ( $items as $i ) {
                $results[] = array(
                    ...
                    'metadata' => array( 'sku' => $i->sku, 'quantity' => $i->quantity ),
    ...
    private function get_facets( $query ) {
        ...
        $facets = $wpdb->get_results( $wpdb->prepare(
    ...
    private function log_search( $query, $entity_type, $result_count ) {
        ...
        $wpdb->insert( $this->search_history_table, array(
            'user_id'      => get_current_user_id(),
            'query'        => $query,
            'entity_type'  => $entity_type,

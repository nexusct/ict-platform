...
class ICT_Calendar_Sync {
	...
	public function register_routes() {
		...
		register_rest_route(
			'ict/v1',
			'/calendar/ical/(?P<token>[a-zA-Z0-9]+)',
			array(
				'methods'             => WP_REST_Server::READABLE,
		...
		register_rest_route(
			'ict/v1',
			'/calendar/oauth/(?P<provider>[a-z]+)/callback',
			array(
				'methods'             => WP_REST_Server::READABLE,
	...
	public function get_connection( $request ) {
		...
		$connection->event_count = $wpdb->get_var(
			$wpdb->prepare(
				"SELECT COUNT(*) FROM {$this->events_table} WHERE connection_id = %d",
				$id
			)
		);

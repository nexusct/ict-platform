...
class ICT_Profitability {
    ...
    public function register_routes() {
        register_rest_route( 'ict/v1', '/profitability/overview', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'get_overview' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/profitability/projects', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'get_project_profitability' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/profitability/projects/(?P<id>\d+)', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'get_project_analysis' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/profitability/employees', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'get_employee_profitability' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/profitability/clients', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'get_client_profitability' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/profitability/trends', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'get_trends' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/profitability/rates', array(
            array(
                'methods'             => WP_REST_Server::READABLE,
        ...
        register_rest_route( 'ict/v1', '/profitability/costs', array(
            array(
                'methods'             => WP_REST_Server::READABLE,
        ...
        register_rest_route( 'ict/v1', '/profitability/forecast', array(
            'methods'             => WP_REST_Server::READABLE,
    ...
    public function get_overview( $request ) {
        ...
        $labor = $wpdb->get_var( $wpdb->prepare(
            "SELECT COALESCE(SUM(
        ...
        $prev_revenue = $wpdb->get_var( $wpdb->prepare(
            "SELECT COALESCE(SUM(total_amount), 0) FROM {$wpdb->prefix}ict_projects
             WHERE status = 'completed' AND end_date BETWEEN %s AND %s",
            $prev_dates['start'], $prev_dates['end']
        ) );
    ...
    public function get_project_analysis( $request ) {
        ...
        $project = $wpdb->get_row( $wpdb->prepare(
            "SELECT * FROM {$wpdb->prefix}ict_projects WHERE id = %d", $id
        ) );
    ...
    public function get_client_profitability( $request ) {
        ...
        foreach ( $clients as $client ) {
            ...
            $project_ids = $wpdb->get_col( $wpdb->prepare(
                ...
                $client->client_id, $dates['start'], $dates['end']
            ) );
    ...
    public function get_trends( $request ) {
        ...
        for ( $i = $months - 1; $i >= 0; $i-- ) {
            ...
            $costs = $wpdb->get_var( $wpdb->prepare(
                "SELECT COALESCE(SUM(amount), 0) FROM {$this->costs_table}
    ...
    private function get_project_costs( $project_id, $detailed = false ) {
        ...
        // Direct costs
        $costs_by_type = $wpdb->get_results( $wpdb->prepare(
        ...
        if ( $detailed ) {
            $result['breakdown'] = $wpdb->get_results( $wpdb->prepare(

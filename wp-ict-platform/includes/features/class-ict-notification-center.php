...
class ICT_Notification_Center {
	...
	private $notification_types = array(
		...
		'time_approved'          => array(
			'label'    => 'Time Entry Approved',
			'icon'     => 'check',
			'category' => 'time',
		),
		'time_rejected'          => array(
			'label'    => 'Time Entry Rejected',
			'icon'     => 'x-circle',
			'category' => 'time',
		),
	...
	public function rest_get_notifications( $request ) {
		...
		if ( $category ) {
			$types_in_category = array_keys(
				array_filter(
					$this->notification_types,
					function ( $t ) use ( $category ) {

...
class ICT_Announcements {
    ...
    public function register_routes() {
        register_rest_route( 'ict/v1', '/announcements', array(
            array(
        ...
        register_rest_route( 'ict/v1', '/announcements/(?P<id>\d+)', array(
            array(
                'methods'             => WP_REST_Server::READABLE,
        ...
        register_rest_route( 'ict/v1', '/announcements/(?P<id>\d+)/read', array(
            'methods'             => WP_REST_Server::CREATABLE,
            'callback'            => array( $this, 'mark_read' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/announcements/(?P<id>\d+)/acknowledge', array(
            'methods'             => WP_REST_Server::CREATABLE,
            'callback'            => array( $this, 'acknowledge' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/announcements/(?P<id>\d+)/stats', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'get_stats' ),
            'permission_callback' => array( $this, 'check_admin_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/announcements/unread', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'get_unread' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/announcements/active', array(
            'methods'             => WP_REST_Server::READABLE,
    ...
    public function create_announcement( $request ) {
        ...
        return rest_ensure_response( array( 'success' => true, 'id' => $ann_id ) );
    }
    ...
    public function update_announcement( $request ) {
        ...
        if ( null !== $content ) $data['content'] = wp_kses_post( $content );

        $target_users = $request->get_param( 'target_users' );
        if ( null !== $target_users ) $data['target_users'] = wp_json_encode( $target_users );
        ...
        if ( null !== $url ) $data['attachment_url'] = esc_url_raw( $url );
    ...
    public function mark_read( $request ) {
        ...
        if ( $existing ) {
            $wpdb->update( $this->reads_table,
                array( 'read_at' => current_time( 'mysql' ) ),
                array( 'id' => $existing )
            );
        } else {
            $wpdb->insert( $this->reads_table, array(
    ...
    public function acknowledge( $request ) {
        ...
        $ann = $wpdb->get_row( $wpdb->prepare(
            "SELECT * FROM {$this->table_name} WHERE id = %d", $ann_id
        ) );
        ...
        if ( $existing ) {
            $wpdb->update( $this->reads_table,
                array( 'read_at' => $now, 'acknowledged_at' => $now ),
                array( 'id' => $existing )
            );
        } else {
            $wpdb->insert( $this->reads_table, array(
    ...
    public function get_stats( $request ) {
        ...
        $readers = $wpdb->get_results( $wpdb->prepare(

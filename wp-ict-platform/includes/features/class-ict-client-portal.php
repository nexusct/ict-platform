...
class ICT_Client_Portal {
    ...
    public function register_routes() {
        ...
        register_rest_route( 'ict/v1', '/portal/projects', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'get_client_projects' ),
            'permission_callback' => array( $this, 'check_client_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/portal/projects/(?P<id>\d+)', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'get_project_details' ),
            'permission_callback' => array( $this, 'check_project_access' ),
        ) );

        register_rest_route( 'ict/v1', '/portal/projects/(?P<id>\d+)/updates', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'get_project_updates' ),
            'permission_callback' => array( $this, 'check_project_access' ),
        ) );

        register_rest_route( 'ict/v1', '/portal/projects/(?P<id>\d+)/documents', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'get_project_documents' ),
            'permission_callback' => array( $this, 'check_project_access' ),
        ) );

        register_rest_route( 'ict/v1', '/portal/projects/(?P<id>\d+)/invoices', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'get_project_invoices' ),
            'permission_callback' => array( $this, 'check_project_access' ),
        ) );

        register_rest_route( 'ict/v1', '/portal/projects/(?P<id>\d+)/request', array(
            'methods'             => WP_REST_Server::CREATABLE,
            'callback'            => array( $this, 'submit_request' ),
            'permission_callback' => array( $this, 'check_project_access' ),
        ) );

        register_rest_route( 'ict/v1', '/portal/requests', array(
            'methods'             => WP_REST_Server::READABLE,
        ...
        // Admin endpoints
        register_rest_route( 'ict/v1', '/portal/access', array(
            array(
                'methods'             => WP_REST_Server::READABLE,
        ...
        register_rest_route( 'ict/v1', '/portal/access/(?P<id>\d+)', array(
            'methods'             => WP_REST_Server::DELETABLE,
            'callback'            => array( $this, 'revoke_access' ),
            'permission_callback' => array( $this, 'check_admin_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/portal/invite', array(
            'methods'             => WP_REST_Server::CREATABLE,
            'callback'            => array( $this, 'invite_client' ),
            'permission_callback' => array( $this, 'check_admin_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/portal/projects/(?P<id>\d+)/post-update', array(
            'methods'             => WP_REST_Server::CREATABLE,
    ...
    public function register_client_role() {
        if ( ! get_role( 'ict_client' ) ) {
            add_role( 'ict_client', 'ICT Client', array(
                'read' => true,
                'ict_client' => true,
            ) );
        }
    ...
    public function get_project_details( $request ) {
        ...
        // Get milestones
        $project->milestones = $wpdb->get_results( $wpdb->prepare(
        ...
        // Get recent updates
        $project->recent_updates = $wpdb->get_results( $wpdb->prepare(
    ...
    public function submit_request( $request ) {
        ...
        return rest_ensure_response( array( 'success' => true, 'id' => $request_id ) );
    }
    ...
    public function post_update( $request ) {
        ...
        return rest_ensure_response( array( 'success' => true, 'id' => $update_id ) );
    }
}

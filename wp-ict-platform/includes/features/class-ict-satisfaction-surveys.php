...
class ICT_Satisfaction_Surveys {
    ...
    public function register_routes() {
        register_rest_route( 'ict/v1', '/surveys', array(
            array(
        ...
        register_rest_route( 'ict/v1', '/surveys/(?P<id>\d+)', array(
            array(
                'methods'             => WP_REST_Server::READABLE,
        ...
        register_rest_route( 'ict/v1', '/surveys/(?P<id>\d+)/questions', array(
            array(
                'methods'             => WP_REST_Server::READABLE,
        ...
        register_rest_route( 'ict/v1', '/surveys/(?P<id>\d+)/send', array(
            'methods'             => WP_REST_Server::CREATABLE,
            'callback'            => array( $this, 'send_survey' ),
            'permission_callback' => array( $this, 'check_edit_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/surveys/(?P<id>\d+)/responses', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'get_responses' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/surveys/(?P<id>\d+)/analytics', array(
            'methods'             => WP_REST_Server::READABLE,
        ...
        register_rest_route( 'ict/v1', '/surveys/respond/(?P<token>[a-zA-Z0-9]+)', array(
            array(
                'methods'             => WP_REST_Server::READABLE,
        ...
        register_rest_route( 'ict/v1', '/surveys/nps-score', array(
            'methods'             => WP_REST_Server::READABLE,
    ...
    public function get_survey( $request ) {
        ...
        $survey = $wpdb->get_row( $wpdb->prepare(
            "SELECT * FROM {$this->surveys_table} WHERE id = %d", $id
        ) );
    ...
    public function create_survey( $request ) {
        ...
        return rest_ensure_response( array( 'success' => true, 'id' => $survey_id ) );
    }
    ...
    public function delete_survey( $request ) {
        ...
        $wpdb->delete( $this->answers_table,
            array( 'response_id' ),
            array( '%d' ),
    ...
    public function send_survey( $request ) {
        ...
        $survey = $wpdb->get_row( $wpdb->prepare(
            "SELECT * FROM {$this->surveys_table} WHERE id = %d", $survey_id
        ) );
        ...
        foreach ( $recipients as $recipient ) {
            ...
            $wpdb->insert( $this->responses_table, array(
                'survey_id'       => $survey_id,
    ...
    public function get_responses( $request ) {
        ...
        foreach ( $responses as $response ) {
            $response->answers = $wpdb->get_results( $wpdb->prepare(
    ...
    public function get_survey_for_response( $request ) {
        ...
        if ( $response->status === 'pending' ) {
            $wpdb->update( $this->responses_table, array(
                'status'     => 'started',
    ...
    public function submit_response( $request ) {
        ...
        foreach ( $answers as $answer ) {
            ...
            $wpdb->insert( $this->answers_table, array(
    ...
    public function get_nps_score( $request ) {
        ...
        if ( $period !== 'all' ) {
            $days = array( '7d' => 7, '30d' => 30, '90d' => 90, '1y' => 365 );
    ...
    private function add_default_nps_questions( $survey_id ) {
        ...
        $wpdb->insert( $this->questions_table, array(
            'survey_id'     => $survey_id,

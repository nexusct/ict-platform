...
class ICT_Push_Notifications {
	...
	public function rest_get_devices( $request ) {
		...
		foreach ( $devices as &$device ) {
			$device->subscriptions = $wpdb->get_col(
				$wpdb->prepare(
	...
	public function rest_get_analytics( $request ) {
		...
		$total_sent_delivery = $wpdb->get_var(
			$wpdb->prepare(
				"SELECT COUNT(*) FROM {$this->tables['delivery']}
		...
		// Messages by day.
		$stats['messages_by_day'] = $wpdb->get_results(
			$wpdb->prepare(

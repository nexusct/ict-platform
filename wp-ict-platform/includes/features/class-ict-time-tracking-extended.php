...
class ICT_Time_Tracking_Extended {
    ...
    public function register_routes() {
        // Break tracking
        register_rest_route( 'ict/v1', '/time-entries/(?P<entry_id>\d+)/breaks', array(
            array(
        ...
        register_rest_route( 'ict/v1', '/breaks/(?P<id>\d+)/end', array(
            'methods'             => WP_REST_Server::CREATABLE,
            'callback'            => array( $this, 'end_break' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/breaks/active', array(
            'methods'             => WP_REST_Server::READABLE,
        ...
        // Overtime
        register_rest_route( 'ict/v1', '/overtime/rules', array(
            array(
                'methods'             => WP_REST_Server::READABLE,
        ...
        register_rest_route( 'ict/v1', '/overtime/rules/(?P<id>\d+)', array(
            array(
        ...
        register_rest_route( 'ict/v1', '/overtime/calculate', array(
            'methods'             => WP_REST_Server::CREATABLE,
            'callback'            => array( $this, 'calculate_overtime_for_period' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/overtime/summary', array(
            'methods'             => WP_REST_Server::READABLE,
        ...
        register_rest_route( 'ict/v1', '/time-approvals', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'get_pending_approvals' ),
            'permission_callback' => array( $this, 'check_manager_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/time-entries/(?P<entry_id>\d+)/submit', array(
            'methods'             => WP_REST_Server::CREATABLE,
            'callback'            => array( $this, 'submit_for_approval' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/time-entries/(?P<entry_id>\d+)/approve', array(
            'methods'             => WP_REST_Server::CREATABLE,
            'callback'            => array( $this, 'approve_time_entry' ),
            'permission_callback' => array( $this, 'check_manager_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/time-entries/(?P<entry_id>\d+)/reject', array(
            'methods'             => WP_REST_Server::CREATABLE,
            'callback'            => array( $this, 'reject_time_entry' ),
            'permission_callback' => array( $this, 'check_manager_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/timesheets/submit', array(
            'methods'             => WP_REST_Server::CREATABLE,
            'callback'            => array( $this, 'submit_timesheet' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/timesheets/approve-batch', array(
            'methods'             => WP_REST_Server::CREATABLE,
    ...
    public function get_overtime_summary( $request ) {
        ...
        return rest_ensure_response( array(
            'period'         => array( 'start' => $start_date, 'end' => $end_date, 'type' => $period ),
    ...
    public function approve_batch( $request ) {
        ...
        foreach ( $entry_ids as $entry_id ) {
            ...
            $wpdb->update(
                ...
                array( 'time_entry_id' => $entry_id, 'status' => 'pending' )
            );

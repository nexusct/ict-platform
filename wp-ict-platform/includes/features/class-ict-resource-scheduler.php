...
class ICT_Resource_Scheduler {
    ...
    public function register_routes() {
        // Resource scheduling
        register_rest_route( 'ict/v1', '/schedules', array(
            array(
        ...
        register_rest_route( 'ict/v1', '/schedules/(?P<id>\d+)', array(
            array(
        ...
        register_rest_route( 'ict/v1', '/schedules/calendar', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'get_calendar_events' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/schedules/conflicts', array(
            'methods'             => WP_REST_Server::CREATABLE,
        ...
        // Availability
        register_rest_route( 'ict/v1', '/availability', array(
            array(
                'methods'             => WP_REST_Server::READABLE,
        ...
        register_rest_route( 'ict/v1', '/availability/users/(?P<user_id>\d+)', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'get_user_availability' ),
            'permission_callback' => array( $this, 'check_permission' ),
        ) );

        register_rest_route( 'ict/v1', '/availability/find', array(
            'methods'             => WP_REST_Server::CREATABLE,
        ...
        // Shifts
        register_rest_route( 'ict/v1', '/shifts', array(
            array(
                'methods'             => WP_REST_Server::READABLE,
        ...
        register_rest_route( 'ict/v1', '/shifts/(?P<id>\d+)', array(
            array(
                'methods'             => WP_REST_Server::EDITABLE,
        ...
        register_rest_route( 'ict/v1', '/shifts/assign', array(
            'methods'             => WP_REST_Server::CREATABLE,
        ...
        // Workload
        register_rest_route( 'ict/v1', '/schedules/workload', array(
            'methods'             => WP_REST_Server::READABLE,
    ...
    public function update_schedule( $request ) {
        ...
        if ( null !== $color ) $data['color'] = sanitize_hex_color( $color );
    ...
    private function find_conflicts( $user_id, $start, $end, $exclude_id = 0 ) {
        ...
        $query = $wpdb->prepare(
            "SELECT * FROM {$this->schedules_table}
             ...
             AND id != %d",
            $user_id, $end, $start, $start, $end, $exclude_id
        );
    ...
    public function get_user_availability( $request ) {
        ...
        $availability = $wpdb->get_results( $wpdb->prepare(
            ...
            $user_id, $date, $day_of_week, $date, $date
        ) );
    ...
    public function assign_shift( $request ) {
        ...
        $existing = $wpdb->get_var( $wpdb->prepare(
            ...
            $data['shift_id'], $data['user_id'], $data['assignment_date']
        ) );

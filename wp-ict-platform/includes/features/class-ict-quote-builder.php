...
class ICT_Quote_Builder {
    ...
    public function register_rest_routes() {
        ...
        // Line items.
        register_rest_route( $namespace, '/quotes/(?P<quote_id>\d+)/items', array(
            array(
                'methods'             => WP_REST_Server::READABLE,
        ...
        register_rest_route( $namespace, '/quote-items/(?P<id>\d+)', array(
            array(
                'methods'             => WP_REST_Server::EDITABLE,
        ...
        // Quote actions.
        register_rest_route( $namespace, '/quotes/(?P<id>\d+)/send', array(
            'methods'             => WP_REST_Server::CREATABLE,
            'callback'            => array( $this, 'rest_send_quote' ),
            'permission_callback' => array( $this, 'check_manage_permission' ),
        ) );

        register_rest_route( $namespace, '/quotes/(?P<id>\d+)/duplicate', array(
            'methods'             => WP_REST_Server::CREATABLE,
            'callback'            => array( $this, 'rest_duplicate_quote' ),
            'permission_callback' => array( $this, 'check_manage_permission' ),
        ) );

        register_rest_route( $namespace, '/quotes/(?P<id>\d+)/new-version', array(
            'methods'             => WP_REST_Server::CREATABLE,
            'callback'            => array( $this, 'rest_create_new_version' ),
            'permission_callback' => array( $this, 'check_manage_permission' ),
        ) );

        register_rest_route( $namespace, '/quotes/(?P<id>\d+)/convert', array(
            'methods'             => WP_REST_Server::CREATABLE,
            'callback'            => array( $this, 'rest_convert_to_project' ),
            'permission_callback' => array( $this, 'check_manage_permission' ),
        ) );

        register_rest_route( $namespace, '/quotes/(?P<id>\d+)/pdf', array(
            'methods'             => WP_REST_Server::READABLE,
        ...
        register_rest_route( $namespace, '/quotes/view/(?P<token>[a-zA-Z0-9]+)', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'rest_view_quote_public' ),
            'permission_callback' => '__return_true',
        ) );

        register_rest_route( $namespace, '/quotes/respond/(?P<token>[a-zA-Z0-9]+)', array(
            'methods'             => WP_REST_Server::CREATABLE,
        ...
        // Templates.
        register_rest_route( $namespace, '/quote-templates', array(
            array(
                'methods'             => WP_REST_Server::READABLE,
        ...
        // Stats.
        register_rest_route( $namespace, '/quotes/stats', array(
            'methods'             => WP_REST_Server::READABLE,
    ...
    public function rest_get_quote( $request ) {
        ...
        // Get line items.
        $quote->line_items = $wpdb->get_results( $wpdb->prepare(
            "SELECT * FROM {$this->tables['line_items']} WHERE quote_id = %d ORDER BY sort_order, id",
            $id
        ) );

        // Get versions.
        $quote->versions = $wpdb->get_results( $wpdb->prepare(
    ...
    public function rest_update_quote( $request ) {
        ...
        $fields = array(
            ...
            'project_name', 'description', 'discount_type', 'discount_value',
            'tax_rate', 'terms_conditions', 'notes', 'internal_notes', 'valid_until'
        );
    ...
    public function rest_update_line_item( $request ) {
        ...
        $fields = array(
            'item_type', 'category', 'description', 'quantity', 'unit',
            'unit_cost', 'markup_percentage', 'is_taxable', 'is_optional', 'sort_order', 'notes'
        );

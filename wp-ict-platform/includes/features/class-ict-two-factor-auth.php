...
class ICT_Two_Factor_Auth {
    ...
    public function register_rest_routes() {
        ...
        // Setup 2FA.
        register_rest_route( $namespace, '/2fa/setup', array(
            array(
        ...
        // Verify setup.
        register_rest_route( $namespace, '/2fa/verify-setup', array(
            'methods'             => WP_REST_Server::CREATABLE,
        ...
        // Disable 2FA.
        register_rest_route( $namespace, '/2fa/disable', array(
            'methods'             => WP_REST_Server::CREATABLE,
        ...
        register_rest_route( $namespace, '/2fa/verify', array(
            'methods'             => WP_REST_Server::CREATABLE,
        ...
        register_rest_route( $namespace, '/2fa/send-code', array(
            'methods'             => WP_REST_Server::CREATABLE,
        ...
        // Backup codes.
        register_rest_route( $namespace, '/2fa/backup-codes', array(
            array(
                'methods'             => WP_REST_Server::READABLE,
        ...
        // Trusted devices.
        register_rest_route( $namespace, '/2fa/trusted-devices', array(
            array(
                'methods'             => WP_REST_Server::READABLE,
        ...
        register_rest_route( $namespace, '/2fa/trusted-devices/(?P<id>\d+)', array(
        ...
        // Status.
        register_rest_route( $namespace, '/2fa/status', array(
            'methods'             => WP_REST_Server::READABLE,
        ...
        // Admin settings.
        register_rest_route( $namespace, '/2fa/settings', array(
            array(
                'methods'             => WP_REST_Server::READABLE,
    ...
    public function rest_get_setup( $request ) {
        ...
        $methods = array(
            array( 'id' => 'totp', 'name' => 'Authenticator App', 'description' => 'Use Google Authenticator or similar app' ),
            array( 'id' => 'email', 'name' => 'Email', 'description' => 'Receive codes via email' ),
        );

        // Add SMS if configured.
        if ( get_option( 'ict_2fa_sms_enabled' ) ) {
            $methods[] = array( 'id' => 'sms', 'name' => 'SMS', 'description' => 'Receive codes via text message' );
        }
    ...
    public function rest_setup_2fa( $request ) {
        ...
        if ( $method === 'totp' ) {
            ...
            if ( $existing ) {
                $wpdb->update(
                    $this->tables['user_2fa'],
                    array( 'method' => 'email', 'is_confirmed' => 1 ),
                    array( 'user_id' => $user_id )
                );
            } else {
                $wpdb->insert(
    ...
    public function rest_verify_setup( $request ) {
        ...
        $user_2fa = $wpdb->get_row( $wpdb->prepare(
            "SELECT * FROM {$this->tables['user_2fa']} WHERE user_id = %d",
            $user_id
        ) );
    ...
    public function rest_get_status( $request ) {
        ...
        $backup_codes = $wpdb->get_var( $wpdb->prepare(
            "SELECT COUNT(*) FROM {$this->tables['backup_codes']}
            WHERE user_id = %d AND is_used = 0",
            $user_id
        ) );

        $trusted_devices = $wpdb->get_var( $wpdb->prepare(

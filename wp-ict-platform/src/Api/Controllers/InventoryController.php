...
class InventoryController extends AbstractController {
	...
	public function registerRoutes(): void {
		...
		register_rest_route(
			...
			array(
				'methods'             => WP_REST_Server::CREATABLE,
				'callback'            => array( $this, 'adjustStock' ),
				'permission_callback' => array( $this, 'updateItemPermissionsCheck' ),
			)
		);
		...
		register_rest_route(
			...
			array(
				'methods'             => WP_REST_Server::READABLE,
				'callback'            => array( $this, 'getLowStock' ),
				'permission_callback' => array( $this, 'getItemsPermissionsCheck' ),
			)
		);
	}
	...
	public function getItems( WP_REST_Request $request ): WP_REST_Response|WP_Error {
		...
		return $this->paginated(
			array_map( array( $this, 'prepareItem' ), $items ),
			$total,
			$pagination['page'],
			$pagination['per_page']
		);
	...
	public function updateItem( WP_REST_Request $request ): WP_REST_Response|WP_Error {
		...
		foreach ( array( 'item_name', 'sku', 'category', 'unit_cost', 'reorder_level' ) as $field ) {
			$value = $request->get_param( $field );
			if ( $value !== null ) {
				$data[ $field ] = in_array( $field, array( 'unit_cost' ) ) ? (float) $value : ( in_array( $field, array( 'reorder_level' ) ) ? (int) $value : sanitize_text_field( $value ) );
			}
		}
		...
		$result = $wpdb->update( ICT_INVENTORY_ITEMS_TABLE, $data, array( 'id' => $id ) );
	...
	public function adjustStock( WP_REST_Request $request ): WP_REST_Response|WP_Error {
		...
		$wpdb->update(
			...
			array( 'quantity_available' => $newQuantity ),
			array( 'id' => $id )
		);
	...
	public function getLowStock( WP_REST_Request $request ): WP_REST_Response|WP_Error {
		...
		return $this->success( array_map( array( $this, 'prepareItem' ), $items ) );
	}

...
class EquipmentController extends AbstractController
{
    ...
    public function registerRoutes(): void
    {
        ...
        register_rest_route(
            ...
            [
                'methods'             => WP_REST_Server::READABLE,
                'callback'            => [$this, 'getDueMaintenance'],
                'permission_callback' => [$this, 'canViewProjects'],
            ]
        );
    ...
    public function updateItem(WP_REST_Request $request): WP_REST_Response|WP_Error
    {
        ...
        $updateable = [
            'equipment_name', 'equipment_number', 'equipment_type', 'manufacturer',
            'model', 'serial_number', 'purchase_date', 'purchase_price', 'current_value',
            'status', 'condition_rating', 'location', 'maintenance_interval_days', 'notes'
        ];
        ...
        foreach ($updateable as $field) {
            if ($request->has_param($field)) {
                ...
                if (in_array($field, ['purchase_price', 'current_value'])) {
                    $data[$field] = (float) $value;
                } elseif (in_array($field, ['condition_rating', 'maintenance_interval_days'])) {
        ...
        $wpdb->update(ICT_EQUIPMENT_TABLE, $data, ['id' => $id]);
    ...
    public function deleteItem(WP_REST_Request $request): WP_REST_Response|WP_Error
    {
        ...
        $wpdb->delete(ICT_EQUIPMENT_TABLE, ['id' => $id]);
        $wpdb->delete(ICT_QR_CODES_TABLE, ['entity_type' => 'equipment', 'entity_id' => $id]);
    ...
    public function scanQrCode(WP_REST_Request $request): WP_REST_Response|WP_Error
    {
        ...
        $wpdb->update(
            ICT_QR_CODES_TABLE,
            [
                'scan_count'       => $wpdb->prepare('scan_count + 1', []),

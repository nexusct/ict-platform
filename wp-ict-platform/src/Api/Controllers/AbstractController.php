            ...
            if (!empty($value)) {
                if (!empty($fieldRules['type'])) {
                    $valid = match ($fieldRules['type']) {
                        ...
                        'bool', 'boolean' => is_bool($value) || in_array($value, ['true', 'false', '0', '1'], true),
                    ...
                    if (!$valid) {
                        return $this->error(
                            'invalid_field_type',
                            sprintf(__('The %s field must be a %s.', 'ict-platform'), $field, $fieldRules['type']),
                            400
                        );
                    }
                }
                ...
                if (!empty($fieldRules['min']) && strlen((string) $value) < $fieldRules['min']) {
                    return $this->error(
                        'field_too_short',
                        sprintf(__('The %s field must be at least %d characters.', 'ict-platform'), $field, $fieldRules['min']),
                        400
                    );
                }
                ...
                if (!empty($fieldRules['max']) && strlen((string) $value) > $fieldRules['max']) {
                    return $this->error(
                        'field_too_long',
                        sprintf(__('The %s field must be at most %d characters.', 'ict-platform'), $field, $fieldRules['max']),
                        400
                    );
                }
                ...
                if (!empty($fieldRules['enum']) && !in_array($value, $fieldRules['enum'], true)) {
                    return $this->error(
                        'invalid_field_value',
                        sprintf(__('The %s field must be one of: %s.', 'ict-platform'), $field, implode(', ', $fieldRules['enum'])),
                        400
                    );
                }
            }

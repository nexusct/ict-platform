...
class ResourceController extends AbstractController
{
    ...
    public function registerRoutes(): void
    {
        ...
        register_rest_route($this->namespace, '/' . $this->rest_base . '/allocations', [
            'methods'             => WP_REST_Server::READABLE,
        ...
        register_rest_route($this->namespace, '/' . $this->rest_base . '/allocations', [
            ...
            'callback'            => [$this, 'createAllocation'],
            'permission_callback' => [$this, 'createItemPermissionsCheck'],
        ]);

        register_rest_route($this->namespace, '/' . $this->rest_base . '/allocations/(?P<id>[\d]+)', [
            'methods'             => WP_REST_Server::EDITABLE,
            'callback'            => [$this, 'updateAllocation'],
        ...
        register_rest_route($this->namespace, '/' . $this->rest_base . '/calendar', [
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => [$this, 'getCalendarEvents'],
            'permission_callback' => [$this, 'getItemsPermissionsCheck'],
        ]);

        register_rest_route($this->namespace, '/' . $this->rest_base . '/conflicts', [
            'methods'             => WP_REST_Server::CREATABLE,
    ...
    public function getItems(WP_REST_Request $request): WP_REST_Response|WP_Error
    {
        ...
        if (!$type || $type === 'technician') {
            $users = get_users(['role__in' => ['ict_technician', 'ict_project_manager']]);
    ...
    public function getAllocations(WP_REST_Request $request): WP_REST_Response|WP_Error
    {
        ...
        return $this->paginated(array_map([$this, 'prepareAllocation'], $allocations), $total, $pagination['page'], $pagination['per_page']);
    }
    ...
    public function updateAllocation(WP_REST_Request $request): WP_REST_Response|WP_Error
    {
        ...
        foreach (['allocation_start', 'allocation_end', 'hours_allocated', 'status'] as $field) {
        ...
        $wpdb->update(ICT_PROJECT_RESOURCES_TABLE, $data, ['id' => $id]);
    ...
    public function deleteAllocation(WP_REST_Request $request): WP_REST_Response|WP_Error
    {
        ...
        $result = $wpdb->delete(ICT_PROJECT_RESOURCES_TABLE, ['id' => (int) $request->get_param('id')]);
    ...
    public function getCalendarEvents(WP_REST_Request $request): WP_REST_Response|WP_Error
    {
        ...
        $events = array_map(function ($allocation) {
            return [
    ...
    public function checkConflicts(WP_REST_Request $request): WP_REST_Response|WP_Error
    {
        ...
        $values = [$resourceType, $resourceId, $end, $start, $end, $start, $start, $end];
        ...
        return $this->success([
            ...
            'conflicts'     => array_map([$this, 'prepareAllocation'], $conflicts),
        ]);
    }

    public function getItemsPermissionsCheck(): bool { return $this->canViewProjects(); }
    public function createItemPermissionsCheck(): bool { return $this->canManageProjects(); }
    public function updateItemPermissionsCheck(): bool { return $this->canManageProjects(); }
    public function deleteItemPermissionsCheck(): bool { return $this->canManageProjects(); }
